...
if m<=11:
	...
	n=6
if pow(0.9,100*error*((n*(n-1))//2))>=n/(100) or (3/11)*m+error*100>=(5*71)/22+40.5:
	print(n)
	...
	for d in dp:
		...
		for j in range(n):
			for k in range(j+1,n):
				if d[count]=="1":
					edges[j].append(str(h[k]))
					edges[k].append(str(h[j]))
				count+=1
		for i in range(n):
			edges[i].sort()
			z[i]=','.join(edges[i])
	...
	for i in range(4,101):
		tmp_score=score[max(i-2,4)]+score[max(i-1,4)]+score[i]+score[min(100,i+1)]+score[min(100,i+2)]
		if max_score<tmp_score:
			corner=i
			num1=(i*(i-1))//2
			num2=(num1)//(m-1)
			max_score=tmp_score
	if max_score*corner>(0.9**2)*9*5:
		corner=int(corner**0.94)
		num1=(corner*(corner-1))//2
		num2=(num1)//(m-1)
	if max_score*corner>(0.9**2)*9*5/3:
		corner=int(corner**0.97)
		num1=(corner*(corner-1))//2
		num2=(num1)//(m-1)
	#print(score)
	print(corner)
	...
	for q in range(100):
		...
		for i in range(m):
			...
			if t<(i+1)*num2*(1-error)+(num1-(i+1)*num2)*(error):
				...
				if min(odd,even)<=3 or max(odd,even)>=m-4:
